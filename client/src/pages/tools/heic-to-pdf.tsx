...
export default function HEICToPDF() {
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      toast({
        title: "No Files",
        description: "Please upload HEIC images",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Success!",
        description: "HEIC images converted to PDF",
      });
    } catch (error) {
      setStatus("error");
      toast({
  ...
  return (
    <ToolLayout
      title="HEIC to PDF"
      description="Upload HEIC images and receive as a PDF file. Convert HEIC images to PDF format."
      category="PDF Tools"
      keywords={["heic to pdf", "convert heic", "heic converter", "images to pdf"]}
      howToSteps={[
        { name: "Upload Images", text: "Upload one or more HEIC images" },
      ...
      <div className="space-y-6">
        <div className="backdrop-blur-md bg-white/70 border border-white/40 rounded-xl p-6">
          ...
          <FileUploader
            ...
            multiple={true}
            allowedTypes={["image/heic", "image/heif"]}
          />
          {files.length > 0 && (
