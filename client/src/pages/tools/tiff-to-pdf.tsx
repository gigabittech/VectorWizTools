...
export default function TIFFToPDF() {
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      toast({
        title: "No Files",
        description: "Please upload TIFF images",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Success!",
        description: "TIFF images converted to PDF",
      });
    } catch (error) {
      setStatus("error");
      toast({
  ...
  return (
    <ToolLayout
      title="TIFF to PDF"
      description="Upload TIFF images and receive as a PDF file. Convert TIFF images to PDF format."
      category="PDF Tools"
      keywords={["tiff to pdf", "convert tiff", "tiff converter", "images to pdf"]}
      howToSteps={[
        { name: "Upload Images", text: "Upload one or more TIFF images" },
      ...
      <div className="space-y-6">
        <div className="backdrop-blur-md bg-white/70 border border-white/40 rounded-xl p-6">
          ...
          <FileUploader
            ...
            multiple={true}
            allowedTypes={["image/tiff", "image/tif"]}
          />
          {files.length > 0 && (
