...
export default function PNGToPDF() {
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      toast({
        title: "No Files",
        description: "Please upload PNG images",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Success!",
        description: "PNG images converted to PDF",
      });
    } catch (error) {
      setStatus("error");
      toast({
  ...
  return (
    <ToolLayout
      title="PNG to PDF"
      description="Upload PNG images and receive as a PDF file. Combine multiple PNG images into a single PDF document."
      category="PDF Tools"
      keywords={["png to pdf", "convert png", "png converter", "images to pdf"]}
      howToSteps={[
        { name: "Upload Images", text: "Upload one or more PNG images" },
