...
export default function AnnotatePDF() {
  ...
  const savePDF = async () => {
    ...
    try {
      ...
      link.download = `annotated_${file.file.name}`;
      link.click();

      toast({ title: "Success", description: "Your annotated PDF has been downloaded." });
    } catch (err) {
      console.error(err);
  ...
  return (
    <ToolLayout
      title="Annotate PDF"
      description="Add drawings, text, and annotations to your PDF documents."
      category="PDF Tools"
      keywords={["annotate pdf", "pdf annotation", "draw on pdf", "sign pdf", "pdf markup"]}
      howToSteps={[
        { name: "Upload PDF", text: "Upload the document you want to annotate" },
        { name: "Select Tool", text: "Choose Pen to draw or Text to type" },
        { name: "Annotate", text: "Draw or type anywhere on the document pages" },
        { name: "Download", text: "Export and download your annotated PDF" },
      ]}
    >
                ...
                <Card className="shadow-lg border-white/40 bg-white/80 backdrop-blur-md">
                  <CardHeader className="pb-3">
                    <CardTitle className="text-lg flex items-center gap-2">
                      <Edit className="h-5 w-5 text-primary" /> Annotation Tools
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 gap-2">
                      ...
                      <Button
                        ...
                        className="justify-start gap-3 h-11"
                      >
                        <PenTool className="h-4 w-4" /> Draw / Sign
                      </Button>
                      <Button
