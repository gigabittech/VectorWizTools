...
export default function WEBPToPDF() {
  ...
  const handleConvert = async () => {
    if (files.length === 0) {
      toast({
        title: "No Files",
        description: "Please upload WEBP images",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Success!",
        description: "WEBP images converted to PDF",
      });
    } catch (error) {
      setStatus("error");
      toast({
  ...
  return (
    <ToolLayout
      title="WEBP to PDF"
      description="Upload WEBP images and receive as a PDF file. Convert WEBP images to PDF format."
      category="PDF Tools"
      keywords={["webp to pdf", "convert webp", "webp converter", "images to pdf"]}
      howToSteps={[
        { name: "Upload Images", text: "Upload one or more WEBP images" },
